...
// 7-5:トップメニュー
// 7-6:クラスとインスタンス
// 7-7:メッセージ表示ルーチンを組み込む
// 7-8:パーティーメンバーのパラメーター
// 7-9:クリーチャーの管理
//7-10:アイテム
// 8-1:探索画面
// 8-2:敵の出現
...
function setup() {
    ...
    var SND = ["menu","dome","event","battle","win","levup","lose","attack","recover"];
    for(var i=0;i<SND.length;i++) loadSound(i,"sound/" + SND[i] + ".m4a");
}

function mainloop() {
    var i,n,r,x,y,w,h,col,btn;
    counter++;
    if(scene < 40) {//背景の表示
    ...
    if(scene==1 || scene==2) {//パーティーメンバーのイラストを表示
        ...
        for(i=0;i<MEMBER_MAX;i++) {
            if(flg[FLG_EVENT+i] > 0) {
                chara[i].pos = chara[i].pos + int((CHR_POS_X[i]-chara[i].pos)/4);
                drawImg(2+i,chara[i].pos,CHR_POS_Y[i]);
            }
        }
    }
    ...
    //確認用のメッセージ表示
    if(debug >= 1) fText("debugmode["+debug+"]",120,30,30,"yellow");//確認用
    if(debug >= 1) fText("scene : "+scene,100,60,30,"black");//確認用
    if(debug >= 1) fText("btl_turn : "+btl_turn,100,90,30,"black");//確認用
    if(debug >= 1) fText("level_m : "+chara[0].level+"  exp:"+chara[0].exp,180,120,30,"black");//確認用
    if(debug >= 1) fText("level_z : "+chara[1].level+"  exp:"+chara[1].exp,180,150,30,"black");//確認用
    if(debug >= 1) fText("level_v : "+chara[2].level+"  exp:"+chara[2].exp,180,180,30,"black");//確認用
    if(debug >= 1) fText("counter : "+counter,100,210,30,"black");//確認用
    ...
    switch(scene) {
        ...
        if(hexaBtn(400,880,360,40,10,"DATA RESET")) {
        ...
        */
        ...
        case 1://ホーム画面
        // if(debug >= 1) fText("ホーム画面に戻るとセーブされます。",400,360,30,"gold");
        // if(debug >= 1) fText("再起動して続きを確認しましょう",400,440,30,"lime");
        putMsg(400,680);
        if(counter == 1) {
            clrMsg();
            autoSave();
        }
        if(counter == 10) {//イベントの発生？
            event = 0;
            if(flg[FLG_EVENT+1]==0 && gtime>=10*60*30) event = FLG_EVENT+1;//総プレイ時間10分でゼノンが加わる
            if(flg[FLG_EVENT+2]==0 && gtime>=20*60*30) event = FLG_EVENT+2;//総プレイ時間20分でビーナスが加わる
            if(flg[FLG_DOME+1]==0 && item[3]>0) event = FLG_DOME+1;//3のアイテム開発でDOME 7へ行けるようになる
            if(flg[FLG_DOME+2]==0 && item[5]>0) event = FLG_DOME+2;//5のアイテム開発でDOME XXXへ行けるようになる
            if(event > 0) {
                scene = 2;
                counter = 0;
            }
        ...
        if(btn > 1) {//メニューボタンが押されたら各画面に遷移する
            //clrMsg();
            scene = (btn-1)*10;
            counter = 0;
        }
        // if(hexaBtn(400,600,240,60,10,"メッセージ確認")) setMsg("メッセージ追加確認" + gtime);
        // putMsg(400,400);
        ...
        */
        ...
        if(counter == 30) {
            if(event == FLG_EVENT+1) {
                chara[1].join(1);
                chara[1].pos = CHR_POS_S[1];
                setMsg("オレの名はゼノン。ドームの探索に協力しよう。");
            }
            if(event == FLG_EVENT+2) {
                chara[2].join(2);
                chara[2].pos = CHR_POS_S[2];
                setMsg("ワタシは強化犬のビーナス。")
                setMsg("ドームの探索に協力しましょう。")
            }
            if(event == FLG_DOME+1 || event == FLG_DOME+2) {
                setMsg("新たなドームへの通路がつながった！");
            }
            flg[event] = 1;
        }
        if(counter < 60) break;
        if(hexaBtn(400,740,160,40,10,"OK")) {
            playBgm(0);
            scene = 1;
            counter = 0;
        }
        ...
        for(i=0;i<MEMBER_MAX;i++) {
            ...
            if(flg[FLG_EVENT+i] > 0) fText(chara[i].name,x,y,30,"white");
        }
        drawFrame(20,100,760,690,"black");
        if(flg[FLG_EVENT+sel_member] > 0) {
            n = 2+sel_member;
            w = img[n].width;
            h = img[n].height;
        ...
        // fText("【制作中】パーティーメンバーの能力確認", 400, 400,30,"yellow");
        ...
        if(flg[FLG_ITEM+sel_item] > 0) fText(ITEM[sel_item*3+1],400,730,30,"white");//説明文
        if(scene == 30) {
            if(counter == 1) {
                for(i=0;i<3;i++) {//完成した回復アイテムが補充される
                    if(flg[FLG_ITEM+i] > 0) item[i]++;
                }
            }
            if(counter == 10) {
                n = int(labo/10);
        ...
        if(counter < 40) break;
        if(counter == 40) {
            if(debug >= 1||rnd(100)<50+20*(item[4]+item[6])) {
                setMsg("クリーチャーを発見！");
                scene = 50;
                counter = 0;
                break;
        ...
        */
        ...
        if(scene == 51) {
            ...
            if(counter == 80) {
                scene = 52;
                if(chara[3].life+chara[4].life+chara[5].life == 0) {
                    scene = 54;
                    counter = 0;
                    break;
                }
                if(chara[0].life+chara[1].life+chara[2].life == 0) {
                    scene = 55;
                    counter = 0;
                    break;
                }
            ...
            if(counter<20 || 60<counter) {//回復を受け付けるタイミング
                ...
                if(btn == 2) {//回復
                    for(i=0;i<3;i++) {
                        if(chara[i].life>0 && chara[i].lfmax && item[i]>0) {
                            item[i]--;
...
var btl_turn = 0;//順に行動する為に用いる
var btl_char = 0;//敵が行動するか
...
//キャラクターの管理
var MEMBER_MAX = 3;//パーティーメンバーの人数
var CHARACTER_MAX = 6;//キャラクターの総数(戦闘時の味方＋敵の数)
...
var CHR_POS_X = [520, -20, 120];//イラスト表示位置　X座標
var CHR_POS_Y = [140, 100, 200];//イラスト表示位置　Y座標
var CHR_POS_S = [800,-200,-200];//画面横から入ってくる初期座標
...
function initVar() {//ゲーム内の変数に初期値を代入
    var i;
    ...
    labo = 0;
    clrMsg();
    chara[0].join(0);//パーティーメンバーのパラメーター代入
    flg[FLG_EVENT+0] = 1;//マリヤが入ったフラグを立てる
    flg[FLG_DOME+0] = 1;//初めに行ける場所
    // chara[1].join(1);
    // chara[2].join(2);    
}

function hexaBtn(x,y,w,h,t,txt) {
...
function ptyLife() {
    ...
    for(i=0;i<MEMBER_MAX;i++) {
        ...
        if(flg[FLG_EVENT+i] > 0) {
            ...
            fText(chara[i].life,x+100,y,28,"white");
        }
...
function initBtl() {
    for(var i=0;i<3;i++) {
        ...
        if(rnd(100) || i==1) setCreature(i, rnd(3)+sel_dome*3);
    }
}
...
function btlOrder() {
    ...
    btl_turn = -1
    i = rnd(6);//順番をランダムに入れ替える
    j = rnd(6);//順番をランダムに入れ替える
    n = order[i];
    order[i] = order[j];
    order[j] = n;
}
